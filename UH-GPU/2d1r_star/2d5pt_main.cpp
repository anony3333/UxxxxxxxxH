...
void cpu_verify()
{
    ...
    for (iter = 0; iter < ITERATION/2; iter++) {
        for (i = 1; i < M-1; i++) {
            for (j = 1; j < N-1; j++) {
                tmp_cpu[i][j] = \
                    0.1*input_cpu[i-1][j] + 0.2*input_cpu[i+1][j] + 
                    0.3*input_cpu[i][j] + 0.4*input_cpu[i][j+1] + 0.5*input_cpu[i][j-1];
            }
        }
        for (i = 1; i < M-1; i++) {
            for (j = 1; j < N-1; j++) {
                input_cpu[i][j] = \
                    0.1*tmp_cpu[i-1][j] + 0.2*tmp_cpu[i+1][j] + 
                    0.3*tmp_cpu[i][j] + 0.4*tmp_cpu[i][j+1] + 0.5*tmp_cpu[i][j-1];
            }
        }
    }
