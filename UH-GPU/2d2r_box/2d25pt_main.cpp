...
extern "C" {void _mlir_ciface_stencil_2d25pt_iteration(struct TwoDMemrefF64 *, struct TwoDMemrefF64 *);}
...
void cpu_verify()
{
    ...
    for (iter = 0; iter < ITERATION/2; iter++) {
        for (i = 2; i < M-2; i++) {
            for (j = 2; j < N-2; j++) {
                tmp_cpu[i][j] = 
                    0.1*input_cpu[i-2][j-2] + 0.2*input_cpu[i-2][j-1] + 0.3*input_cpu[i-2][j] + 
                    0.4*input_cpu[i-2][j+1] + 0.5*input_cpu[i-2][j+2] +

                    0.1*input_cpu[i-1][j-2] + 0.2*input_cpu[i-1][j-1] + 0.3*input_cpu[i-1][j] + 
                    0.4*input_cpu[i-1][j+1] + 0.5*input_cpu[i-1][j+2] +

                    0.1*input_cpu[i][j-2] + 0.2*input_cpu[i][j-1] + 0.3*input_cpu[i][j] + 
                    0.4*input_cpu[i][j+1] + 0.5*input_cpu[i][j+2] +
        ...
        for (i = 2; i < M-2; i++) {
            for (j = 2; j < N-2; j++) {
                input_cpu[i][j] = 
                    0.1*tmp_cpu[i-2][j-2] + 0.2*tmp_cpu[i-2][j-1] + 0.3*tmp_cpu[i-2][j] + 
                    0.4*tmp_cpu[i-2][j+1] + 0.5*tmp_cpu[i-2][j+2] +

                    0.1*tmp_cpu[i-1][j-2] + 0.2*tmp_cpu[i-1][j-1] + 0.3*tmp_cpu[i-1][j] + 
                    0.4*tmp_cpu[i-1][j+1] + 0.5*tmp_cpu[i-1][j+2] +

                    0.1*tmp_cpu[i][j-2] + 0.2*tmp_cpu[i][j-1] + 0.3*tmp_cpu[i][j] + 
                    0.4*tmp_cpu[i][j+1] + 0.5*tmp_cpu[i][j+2] +
...
int main(int argc, char *argv[])
{
    ...
    tic();
    _mlir_ciface_stencil_2d25pt_iteration(&A_mem, &B_mem);
    cudaDeviceSynchronize();
    double elapsedTime = tok();
