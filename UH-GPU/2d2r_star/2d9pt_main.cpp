...
void cpu_verify()
{
    ...
    for (iter = 0; iter < ITERATION/2; iter++) {
        for (i = 2; i < M-2; i++) {
            for (j = 2; j < N-2; j++) {
                tmp_cpu[i][j] = 
                    0.1*input_cpu[i-2][j] + 0.2*input_cpu[i+2][j] + 
                    0.3*input_cpu[i-1][j] + 0.4*input_cpu[i+1][j] + 
                    0.5*input_cpu[i][j] + 
                    0.6*input_cpu[i][j+1] + 0.7*input_cpu[i][j-1] +
                    0.8*input_cpu[i][j+2] + 0.9*input_cpu[i][j-2];
            }
        }
        for (i = 2; i < M-2; i++) {
            for (j = 2; j < N-2; j++) {
                input_cpu[i][j] = 
                    0.1*tmp_cpu[i-2][j] + 0.2*tmp_cpu[i+2][j] + 
                    0.3*tmp_cpu[i-1][j] + 0.4*tmp_cpu[i+1][j] + 
                    0.5*tmp_cpu[i][j] + 
                    0.6*tmp_cpu[i][j+1] + 0.7*tmp_cpu[i][j-1] +
                    0.8*tmp_cpu[i][j+2] + 0.9*tmp_cpu[i][j-2];
            }
        }
    }
