...
void cpu_verify()
{
    ...
    for (iter = 0; iter < ITERATION/2; iter++) {
        for (i = 3; i < M-3; i++) {
            for (j = 3; j < N-3; j++) {
                tmp_cpu[i][j] = 
                    0.1*input_cpu[i-3][j] + 0.1*input_cpu[i+3][j] + 
                    0.1*input_cpu[i-2][j] + 0.1*input_cpu[i+2][j] + 
                    0.1*input_cpu[i-1][j] + 0.1*input_cpu[i+1][j] + 
                    0.1*input_cpu[i][j] + 
                    0.1*input_cpu[i][j+1] + 0.1*input_cpu[i][j-1] +
                    0.1*input_cpu[i][j+2] + 0.1*input_cpu[i][j-2] +
                    0.1*input_cpu[i][j+3] + 0.1*input_cpu[i][j-3];
            }
        }
        for (i = 3; i < M-3; i++) {
            for (j = 3; j < N-3; j++) {
                input_cpu[i][j] = 
                    0.1*tmp_cpu[i-3][j] + 0.1*tmp_cpu[i+3][j] + 
                    0.1*tmp_cpu[i-2][j] + 0.1*tmp_cpu[i+2][j] + 
                    0.1*tmp_cpu[i-1][j] + 0.1*tmp_cpu[i+1][j] + 
                    0.1*tmp_cpu[i][j] + 
                    0.1*tmp_cpu[i][j+1] + 0.1*tmp_cpu[i][j-1] +
                    0.1*tmp_cpu[i][j+2] + 0.1*tmp_cpu[i][j-2] +
                    0.1*tmp_cpu[i][j+3] + 0.1*tmp_cpu[i][j-3];
            }
        }
    }
