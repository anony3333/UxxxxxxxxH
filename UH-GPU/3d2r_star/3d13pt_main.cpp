...
extern "C" {void _mlir_ciface_stencil_3d13pt_iteration(struct TwoDMemrefF64 *, struct TwoDMemrefF64 *);}
    ...
    for (iter = 0; iter < ITERATION/2; iter++) {
        for (i = 2; i < M-2; i++) {
            for (j = 2; j < N-2; j++) {
                for (k = 2; k < Q-2; k++) {
                    tmp_cpu[i][j][k] = 
                        0.1*input_cpu[i-2][j][k] + 0.2*input_cpu[i+2][j][k] +
                        0.3*input_cpu[i][j-2][k] + 0.4*input_cpu[i][j+2][k] +
                        0.5*input_cpu[i][j][k-2] + 0.6*input_cpu[i][j][k+2] +

                        0.1*input_cpu[i-1][j][k] + 0.2*input_cpu[i+1][j][k] +
                        0.3*input_cpu[i][j-1][k] + 0.4*input_cpu[i][j+1][k] +
                        0.5*input_cpu[i][j][k-1] + 0.6*input_cpu[i][j][k+1] +
                        0.7*input_cpu[i][j][k];
                }
            }
        }
        for (i = 2; i < M-2; i++) {
            for (j = 2; j < N-2; j++) {
                for (k = 2; k < Q-2; k++) {
                    input_cpu[i][j][k] = 
                        0.1*tmp_cpu[i-2][j][k] + 0.2*tmp_cpu[i+2][j][k] +
                        0.3*tmp_cpu[i][j-2][k] + 0.4*tmp_cpu[i][j+2][k] +
                        0.5*tmp_cpu[i][j][k-2] + 0.6*tmp_cpu[i][j][k+2] +

                        0.1*tmp_cpu[i-1][j][k] + 0.2*tmp_cpu[i+1][j][k] +
                        0.3*tmp_cpu[i][j-1][k] + 0.4*tmp_cpu[i][j+1][k] +
                        0.5*tmp_cpu[i][j][k-1] + 0.6*tmp_cpu[i][j][k+1] +
                        0.7*tmp_cpu[i][j][k];
                }
            }
        }
...
int main(int argc, char *argv[])
{
    ...
    tic();
    _mlir_ciface_stencil_3d13pt_iteration(&A_mem, &B_mem);
    cudaDeviceSynchronize();
    double elapsedTime = tok();
