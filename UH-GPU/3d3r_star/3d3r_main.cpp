    ...
    for (iter = 0; iter < ITERATION/2; iter++) {
        for (i = 3; i < M-3; i++) {
            for (j = 3; j < N-3; j++) {
                for (k = 3; k < Q-3; k++) {
                    tmp_cpu[i][j][k] = 
                        0.1*input_cpu[i-3][j][k] + 0.1*input_cpu[i+3][j][k] +
            0.1*input_cpu[i][j-3][k] + 0.1*input_cpu[i][j+3][k] +
            0.1*input_cpu[i][j][k-3] + 0.1*input_cpu[i][j][k+3] +

            0.1*input_cpu[i-2][j][k] + 0.1*input_cpu[i+2][j][k] +
            0.1*input_cpu[i][j-2][k] + 0.1*input_cpu[i][j+2][k] +
            0.1*input_cpu[i][j][k-2] + 0.1*input_cpu[i][j][k+2] +

            0.1*input_cpu[i-1][j][k] + 0.1*input_cpu[i+1][j][k] +
            0.1*input_cpu[i][j-1][k] + 0.1*input_cpu[i][j+1][k] +
            0.1*input_cpu[i][j][k-1] + 0.1*input_cpu[i][j][k+1] +
            0.1*input_cpu[i][j][k];
                }
            }
        }
        for (i = 3; i < M-3; i++) {
            for (j = 3; j < N-3; j++) {
                for (k = 3; k < Q-3; k++) {
                    input_cpu[i][j][k] = 
                    0.1*tmp_cpu[i-3][j][k] + 0.1*tmp_cpu[i+3][j][k] +
            0.1*tmp_cpu[i][j-3][k] + 0.1*tmp_cpu[i][j+3][k] +
            0.1*tmp_cpu[i][j][k-3] + 0.1*tmp_cpu[i][j][k+3] +

            0.1*tmp_cpu[i-2][j][k] + 0.1*tmp_cpu[i+2][j][k] +
            0.1*tmp_cpu[i][j-2][k] + 0.1*tmp_cpu[i][j+2][k] +
            0.1*tmp_cpu[i][j][k-2] + 0.1*tmp_cpu[i][j][k+2] +

            0.1*tmp_cpu[i-1][j][k] + 0.1*tmp_cpu[i+1][j][k] +
            0.1*tmp_cpu[i][j-1][k] + 0.1*tmp_cpu[i][j+1][k] +
            0.1*tmp_cpu[i][j][k-1] + 0.1*tmp_cpu[i][j][k+1] +
            0.1*tmp_cpu[i][j][k];
