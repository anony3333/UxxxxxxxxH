...
void mpe_verify()
{
    ...
    for (iter = 0; iter < ITERATION/2; iter++) {
        for (i = 1; i < M-1; i++) {
            for (j = 1; j < N-1; j++) {
                    tmp_mpe[i][j] = 
            0.1*input_mpe[i-1][j-1] + 0.1*input_mpe[i+1][j-1] + 
            0.2*input_mpe[i-1][j] + 0.2*input_mpe[i+1][j] + 
            0.3*input_mpe[i-1][j+1] + 0.3*input_mpe[i+1][j+1] + 
            0.4*input_mpe[i][j] + 0.5*input_mpe[i][j+1] + 0.6*input_mpe[i][j-1];
            }
        }
        for (i = 1; i < M-1; i++) {
            for (j = 1; j < N-1; j++) {
                    input_mpe[i][j] = 
            0.1*tmp_mpe[i-1][j-1] + 0.1*tmp_mpe[i+1][j-1] + 
            0.2*tmp_mpe[i-1][j] + 0.2*tmp_mpe[i+1][j] + 
            0.3*tmp_mpe[i-1][j+1] + 0.3*tmp_mpe[i+1][j+1] + 
            0.4*tmp_mpe[i][j] + 0.5*tmp_mpe[i][j+1] + 0.6*tmp_mpe[i][j-1]; 
            }
        }
