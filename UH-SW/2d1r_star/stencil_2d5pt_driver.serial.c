...
void stencil_2d5pt_iteration(double value_arg0[M][N], double value_arg1[M][N]);
...
void mpe_verify()
{
    ...
    for (iter = 0; iter < ITERATION/2; iter++) {
        ...
        for (i = 1; i < M-1; i++) {
            for (j = 1; j < N-1; j++) {
                    input_mpe[i][j] = 0.1*tmp_mpe[i-1][j] + 0.2*tmp_mpe[i+1][j] + 
...
int main(int argc, char *argv[])
{
    ...
    tic();
    stencil_2d5pt_iteration(input_spe, tmp_spe);
    double elapsedTime = tok();
