...
void stencil_3d27pt_iteration(double value_arg0[M][N][Q], double value_arg1[M][N][Q]);
...
void mpe_verify()
{
    ...
    for (iter = 0; iter < ITERATION/2; iter++) {
        for (i = 1; i < M-1; i++) {
            ...
            0.1*input_mpe[i-1][j][k-1] + 0.2*input_mpe[i-1][j][k] + 0.3*input_mpe[i-1][j][k+1] +
            0.1*input_mpe[i-1][j+1][k-1] + 0.2*input_mpe[i-1][j+1][k] + 0.3*input_mpe[i-1][j+1][k+1] +

            0.1*input_mpe[i][j-1][k-1] + 0.2*input_mpe[i][j-1][k] + 0.3*input_mpe[i][j-1][k+1] + 
            0.1*input_mpe[i][j][k-1] + 0.2*input_mpe[i][j][k] + 0.3*input_mpe[i][j][k+1] +
            0.1*input_mpe[i][j+1][k-1] + 0.2*input_mpe[i][j+1][k] + 0.3*input_mpe[i][j+1][k+1] +
        ...
        for (i = 1; i < M-1; i++) {
            for (j = 1; j < N-1; j++) {
                for (k = 1; k < Q-1; k++) {
            input_mpe[i][j][k] = 0.1*tmp_mpe[i-1][j-1][k-1] + 0.2*tmp_mpe[i-1][j-1][k] + 0.3*tmp_mpe[i-1][j-1][k+1] + 
                0.1*tmp_mpe[i-1][j][k-1] + 0.2*tmp_mpe[i-1][j][k] + 0.3*tmp_mpe[i-1][j][k+1] +
                ...
                0.1*tmp_mpe[i][j][k-1] + 0.2*tmp_mpe[i][j][k] + 0.3*tmp_mpe[i][j][k+1] +
                0.1*tmp_mpe[i][j+1][k-1] + 0.2*tmp_mpe[i][j+1][k] + 0.3*tmp_mpe[i][j+1][k+1] +
...
int main(int argc, char *argv[])
{
    ...
    tic();
    stencil_3d27pt_iteration(input_spe, tmp_spe);
    double elapsedTime = tok();
