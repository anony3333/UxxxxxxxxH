...
void stencil_2d9pt_star_iteration(double value_arg0[M][N], double value_arg1[M][N]);
    ...
    for (iter = 0; iter < ITERATION/2; iter++) {
        for (i = HALO; i < M-HALO; i++) {
            for (j = HALO; j < N-HALO; j++) {
                tmp_mpe[i][j] = \
                    0.1*input_mpe[i-2][j] + 0.2*input_mpe[i-1][j] + 0.3*input_mpe[i][j-2]
                    + 0.4*input_mpe[i][j-1] + 0.5*input_mpe[i][j] + 0.6*input_mpe[i][j+1]
                    + 0.7*input_mpe[i][j+2] + 0.8*input_mpe[i+1][j] + 0.9*input_mpe[i+2][j];
            }
        }
        for (i = HALO; i < M-HALO; i++) {
            for (j = HALO; j < N-HALO; j++) {
                input_mpe[i][j] = \
                    0.1*tmp_mpe[i-2][j] + 0.2*tmp_mpe[i-1][j] + 0.3*tmp_mpe[i][j-2]
                    + 0.4*tmp_mpe[i][j-1] + 0.5*tmp_mpe[i][j] + 0.6*tmp_mpe[i][j+1]
                    + 0.7*tmp_mpe[i][j+2] + 0.8*tmp_mpe[i+1][j] + 0.9*tmp_mpe[i+2][j];
            }
        }
    }
...
int main(int argc, char *argv[])
{
    ...
    tic();
    stencil_2d9pt_star_iteration(input_spe, tmp_spe);
    double elapsedTime = tok();
