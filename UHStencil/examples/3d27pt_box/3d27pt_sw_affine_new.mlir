loc("stencil_3d27pt_box.dsl":7:12): error: copy generation failed for one or more memref's in this block
...
module  {
  func @stencil_3d27pt_box(%arg0: memref<258x258x258xf64>, %arg1: memref<258x258x258xf64>) attributes {hardware = "sw", llvm.emit_c_interface, std.stencil_program} {
    affine.parallel (%arg2, %arg3, %arg4) = (0, 0, 0) to (4, 4, 4) {
      affine.for %arg5 = affine_map<(d0) -> (d0 * 64 + 1)>(%arg4) to affine_map<(d0) -> (d0 * 64 + 65)>(%arg4) step 4 {
        affine.for %arg6 = affine_map<(d0) -> (d0 * 64 + 1)>(%arg3) to affine_map<(d0) -> (d0 * 64 + 65)>(%arg3) step 4 {
          affine.for %arg7 = affine_map<(d0) -> (d0 * 64 + 1)>(%arg2) to affine_map<(d0) -> (d0 * 64 + 65)>(%arg2) step 4 {
            affine.for %arg8 = 0 to 4 {
              affine.for %arg9 = 0 to 4 {
                affine.for %arg10 = 0 to 4 {
                  %0 = affine.apply affine_map<(d0, d1) -> (d0 + d1)>(%arg5, %arg8)
                  %1 = affine.apply affine_map<(d0, d1) -> (d0 + d1)>(%arg6, %arg9)
                  %2 = affine.apply affine_map<(d0, d1) -> (d0 + d1)>(%arg7, %arg10)
          ...
          } {dim_cpe = 2 : index, domain_lower_bound = 1 : index, thread_inner_loop, thread_loop}
        } {dim_cpe = 1 : index, domain_lower_bound = 1 : index, thread_loop}
      } {dim_cpe = 0 : index, domain_lower_bound = 1 : index, thread_loop}
    }
    return
  }
