...
void mlir::populateAffineToStdPartiallyConversionPatterns(RewritePatternSet &patterns) {
  // clang-format off
  patterns.add<    
      AffineApplyLowering,
...
namespace {
class LowerAffinePartiallyPass : public LowerAffinePartiallyPassBase<LowerAffinePartiallyPass> {
  void runOnOperation() override {
    RewritePatternSet patterns(&getContext());
    populateAffineToStdPartiallyConversionPatterns(patterns);
...
/// equivalent blocks.
std::unique_ptr<Pass> mlir::createLowerAffinePartiallyPass() {
  return std::make_unique<LowerAffinePartiallyPass>();
}

void mlir::populateAffineParallelToSCFConversionPatterns(RewritePatternSet &patterns) {
  // clang-format off
  patterns.add<    
      // AffineApplyLowering,
      // AffineDmaStartLowering,
...
namespace {
class LowerAffineParallelToSCFPass : public LowerAffineParallelToSCFPassBase<LowerAffineParallelToSCFPass> {
  void runOnOperation() override {
    RewritePatternSet patterns(&getContext());
    populateAffineParallelToSCFConversionPatterns(patterns);
    // populateAffineToVectorConversionPatterns(patterns);
    ConversionTarget target(getContext());
    target.addLegalDialect<memref::MemRefDialect, scf::SCFDialect,
                           StandardOpsDialect, VectorDialect>();
...
/// Lowers If and For operations within a function into their lower level CFG
/// equivalent blocks.
std::unique_ptr<Pass> mlir::createLowerAffineParallelToSCFPass() {
  return std::make_unique<LowerAffineParallelToSCFPass>();
}