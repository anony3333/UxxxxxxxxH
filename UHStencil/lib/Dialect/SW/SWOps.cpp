...
static void printAttributions(OpAsmPrinter &printer, ArrayRef<BlockArgument> values)
{
    ...
    // auto elemType = values[0].getType().cast<mlir::MemRefType>().getElementType();
    ...
    llvm::interleaveComma(values, printer, [&](BlockArgument v){
        ...
        // for (auto size : v.getType().cast<mlir::MemRefType>().getShape()) {
        //     printer <<"[" << size << "]";
        // }
    });
...
static void print(sw::FuncOp funcOp, OpAsmPrinter &printer)
{
    ...
    for (unsigned iter = 0; iter < argTypes.size(); iter++) {
        ...
        // auto elemType = argTypes[iter].cast<mlir::MemRefType>().getElementType();
        if (elemType.cast<mlir::FloatType>().getWidth() == 64)
            printer << "double *";
        else
            printer << "float *";
    ...
    for (unsigned iter = 0; iter < argTypes.size(); iter++) {
        ...
        // auto shape = argTypes[iter].cast<mlir::MemRefType>().getShape();
...
{
    auto elemType = forOp.getInductionVar().getType();
    // 定义循环变量
    if (elemType.isa<mlir::IndexType>())
        printer << "long ";
    else if (elemType.cast<mlir::IntegerType>().getWidth() == 64)
        printer << "long ";
    else
        printer << "int ";
...
static void print(sw::GetIDOp getIDOp, OpAsmPrinter &printer)
{
    ...
    if (elemType.isa<mlir::IntegerType>() || elemType.isa<mlir::IndexType>()) {
...
static void print(sw::AddiOp addiOp, OpAsmPrinter &printer)
{
    ...
    auto elemType = addiOp.res().getType();
    // 输出结果类型
    if (elemType.isa<mlir::IndexType>())
        printer << "$moveToHead<-long";
    else if (elemType.cast<mlir::IntegerType>().getWidth() == 64)
...
static void print(sw::SubiOp subiOp, OpAsmPrinter &printer)
{
    ...
    auto elemType = subiOp.res().getType();
    // 输出结果类型
    if (elemType.isa<mlir::IndexType>())
        printer << "$moveToHead<-long";
    else if (elemType.cast<mlir::IntegerType>().getWidth() == 64)
...
static void print(sw::MuliOp muliOp, OpAsmPrinter &printer)
{
    ...
    auto elemType = muliOp.res().getType();
    // 输出结果类型
    if (elemType.isa<mlir::IndexType>())
        printer << "$moveToHead<-long";
    else if (elemType.cast<mlir::IntegerType>().getWidth() == 64)
...
//============================================================================//
// 解析函数
static ParseResult parseDiviOp(OpAsmParser &parser, OperationState &state)
{
    return parseMathOp(parser, state);
}

// 输出函数
static void print(sw::DiviOp diviOp, OpAsmPrinter &printer)
{
    printer << diviOp.lhs() << " / " << diviOp.rhs() << ";";

    auto elemType = diviOp.res().getType();
    // 输出结果类型
    if (elemType.isa<mlir::IndexType>())
        printer << "$moveToHead<-long";
    else if (elemType.cast<mlir::IntegerType>().getWidth() == 64)
...
static void print(sw::MemcpyToLDMOp memcpyToLDMOp, OpAsmPrinter &printer)
{
    ...
    // auto elemType = memcpyToLDMOp.mem_addr().getType().cast<mlir::MemRefType>().getElementType();
...
static void print(sw::MemcpyToMEMOp memcpyToMEMOp, OpAsmPrinter &printer)
{
    ...
    // auto elemType = memcpyToMEMOp.mem_addr().getType().cast<mlir::MemRefType>().getElementType();
    auto TypeWidth = (elemType.cast<mlir::FloatType>().getWidth() == 64) ?
                    "*sizeof(double)" : "*sizeof(float)";
...
{
    auto resultType = allocOp.getResult().getType().cast<mlir::sw::GridType>();
    // auto resultType = allocOp.getResult().getType().cast<mlir::MemRefType>();
...
static void print(sw::MpiExchangeHaloOp mpiExchangeHaloOp, OpAsmPrinter &printer)
{
    ...
    // auto arrayType = mpiExchangeHaloOp.dataArray().getType().cast<mlir::MemRefType>();
    // auto arrayDim = arrayType.getRank();
    auto arrayDim = arrayType.getShape().size();
