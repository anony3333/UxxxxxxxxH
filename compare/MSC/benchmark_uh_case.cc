...
//modify single precision to double precision
#define DT f64
//

void bench_2d5pt_star(std::string target, int num_threads, int D1, int D2, 
  ...
  //
  Kernel A( (k,j) , c0* B[k,j] + c1* B[k,j-1] + c2* B[k,j+1] + c3* B[k-1,j] + c4* B[k+1,j] , schedule );
  //
  if (use_schedule){
    Axis xo, yo, xi, yi;
  ...
  char func_name[100];
  sprintf(func_name, "bench_2d5pt_star_%s_%d_%d_%d_%d_%d_%d_%d_%d", target.c_str(), num_threads, D1, D2, M, N, tile_size_x, tile_size_y, is_assigned);
  stencil.compile_to_source_code_mpi(func_name, target, use_timing);
}


/*
stencil is a 2D 25-point stencil
            out = 0.1*input[y-2][x-2] + 0.2*input[y-2][x-1] + 0.3*input[y-2][x] + 
...
*/
// yxy add  
void bench_2d25pt_box(std::string target, int num_threads, int D1, int D2,
        int M, int N, int tile_size_x,  int tile_size_y, int use_schedule=1, int is_assigned=0) {
  init();
  ...
  DefVar(j,i32);
  // 0
  DefVar_Value(c0_0, DT, 0.1);
  DefVar_Value(c0_1, DT, 0.2);
  DefVar_Value(c0_2, DT, 0.3);
  DefVar_Value(c0_3, DT, 0.4);
  DefVar_Value(c0_4, DT, 0.5);
  // 1
  DefVar_Value(c1_0, DT, 0.1);
  DefVar_Value(c1_1, DT, 0.2);
  DefVar_Value(c1_2, DT, 0.3);
  DefVar_Value(c1_3, DT, 0.4);
  DefVar_Value(c1_4, DT, 0.5);
  // 2
  DefVar_Value(c2_0, DT, 0.1);
  DefVar_Value(c2_1, DT, 0.2);
  DefVar_Value(c2_2, DT, 0.3);
  DefVar_Value(c2_3, DT, 0.4);
  DefVar_Value(c2_4, DT, 0.5);
  // 3
  DefVar_Value(c3_0, DT, 0.1);
  DefVar_Value(c3_1, DT, 0.2);
  DefVar_Value(c3_2, DT, 0.3);
  DefVar_Value(c3_3, DT, 0.4);
  DefVar_Value(c3_4, DT, 0.5);
  // 4
  DefVar_Value(c4_0, DT, 0.1);
  DefVar_Value(c4_1, DT, 0.2);
  DefVar_Value(c4_2, DT, 0.3);
  DefVar_Value(c4_3, DT, 0.4);
  DefVar_Value(c4_4, DT, 0.5);

  //
  Kernel A((k, j), c0_0 * B[k-2, j-2] + c0_1 * B[k-2, j-1] + c0_2 * B[k-2, j] + c0_3 * B[k-2, j+1] + c0_4 * B[k-2, j+2] +
            c1_0 * B[k-1, j-2] + c1_1 * B[k-1, j-1] + c1_2 * B[k-1, j] + c1_3 * B[k-1, j+1] + c1_4 * B[k-1, j+2] +
            c2_0 * B[k, j-2] + c2_1 * B[k, j-1] + c2_2 * B[k, j] + c2_3 * B[k, j+1] + c2_4 * B[k, j+2] +
            c3_0 * B[k+1, j-2] + c3_1 * B[k+1, j-1] + c3_2 * B[k+1, j] + c3_3 * B[k+1, j+1] + c3_4 * B[k+1, j+2] +
            c4_0 * B[k+2, j-2] + c4_1 * B[k+2, j-1] + c4_2 * B[k+2, j] + c4_3 * B[k+2, j+1] + c4_4 * B[k+2, j+2], schedule);
  //
  ...
  const bool use_timing = true;
  char func_name[100];
  sprintf(func_name, "2d25pt%s_%d_%d_%d_%d_%d_%d_%d_%dschedule", target.c_str(), num_threads, D1,D2, M,N, tile_size_x, tile_size_y, use_schedule);
  stencil.compile_to_source_code_mpi(func_name, target, use_timing);
...
/*yxy add: radius=3, 2d13pt*/
void bench_2d13pt_star(std::string target, int num_threads, int D1, int D2,
        int M, int N, int tile_size_x,  int tile_size_y, int use_schedule=1, int is_assigned=0){
  init();
  ...
  DefVar_Value(c3, DT, 0.4);
  DefVar_Value(c4, DT, 0.5);
  DefVar_Value(c5, DT, 0.6);
  DefVar_Value(c6, DT, 0.7);
  DefVar_Value(c7, DT, 0.8);
  DefVar_Value(c8, DT, 0.9);
  DefVar_Value(c9, DT, 0.9);
  DefVar_Value(c10, DT, 1.0);
  DefVar_Value(c11, DT, 1.1);
  DefVar_Value(c12, DT, 1.2);
  //
  // Kernel A( (k,j) , c0* B[k,j] + c1* B[k,j-1] + c2* B[k,j-2] + c3* B[k,j+1] + c4* B[k,j+2] + c5* B[k-1,j] + c6* B[k-2,j] + c7* B[k+1,j] + c8* B[k+2,j] , schedule );
  Kernel A( (k,j), c0 * B[k-3, j] + c1 * B[k-2, j] + c2 * B[k-1, j] + c3 * B[k, j] + c4 * B[k+1, j] + c5 * B[k+2, j] + c6 * B[k+3, j] + c7 * B[k, j-3] + c8 * B[k, j-2] + c9 * B[k, j-1] + c10 * B[k, j+1] + c11 * B[k, j+2] + c12 * B[k, j+3], schedule);
  //
  if (use_schedule){
    Axis xo, yo, xi, yi;
    A.tile(tile_size_x, tile_size_y, xo, xi, yo, yi);
  ...
  const bool use_timing = true;
  char func_name[100];
  sprintf(func_name, "2d13pt_star_%s_%d_%d_%d_%d_%d_%d_%d_%dschedule", target.c_str(), num_threads, D1,D2, M,N, tile_size_x, tile_size_y, use_schedule);
  stencil.compile_to_source_code_mpi(func_name, target, use_timing);
}
...
/*yxy add: radius=3, box, 2d49pt*/
void bench_2d49pt_box(std::string target, int num_threads, int D1, int D2,
  int M, int N, int tile_size_x,  int tile_size_y, int use_schedule=1, int is_assigned=0){
  init();
  const int halo_size = 3;
  const int time_window_size = 2;
  ...
  DefVar_Value(c6, DT, 0.7);
  //
  Kernel A( (k,j) , c0 * B[k-3, j-3] + c1 * B[k-3, j-2] + c2 * B[k-3, j-1] + c3 * B[k-3, j] + c4 * B[k-3, j+1] + c5 * B[k-3, j+2] + c6 * B[k-3, j+3] +
            c0 * B[k-2, j-3] + c1 * B[k-2, j-2] + c2 * B[k-2, j-1] + c3 * B[k-2, j] + c4 * B[k-2, j+1] + c5 * B[k-2, j+2] + c6 * B[k-2, j+3] +
            c0 * B[k-1, j-3] + c1 * B[k-1, j-2] + c2 * B[k-1, j-1] + c3 * B[k-1, j] + c4 * B[k-1, j+1] + c5 * B[k-1, j+2] + c6 * B[k-1, j+3] +
            c0 * B[k,   j-3] + c1 * B[k,   j-2] + c2 * B[k,   j-1] + c3 * B[k,   j] + c4 * B[k,   j+1] + c5 * B[k,   j+2] + c6 * B[k,   j+3] +
            c0 * B[k+1, j-3] + c1 * B[k+1, j-2] + c2 * B[k+1, j-1] + c3 * B[k+1, j] + c4 * B[k+1, j+1] + c5 * B[k+1, j+2] + c6 * B[k+1, j+3] +
            c0 * B[k+2, j-3] + c1 * B[k+2, j-2] + c2 * B[k+2, j-1] + c3 * B[k+2, j] + c4 * B[k+2, j+1] + c5 * B[k+2, j+2] + c6 * B[k+2, j+3] +
            c0 * B[k+3, j-3] + c1 * B[k+3, j-2] + c2 * B[k+3, j-1] + c3 * B[k+3, j] + c4 * B[k+3, j+1] + c5 * B[k+3, j+2] + c6 * B[k+3, j+3], schedule);
  if (use_schedule){
    Axis xo, yo, xi, yi;
    A.tile(tile_size_x, tile_size_y, xo, xi, yo, yi);
    A.reorder(xo, yo, xi, yi);
  ...
  const bool use_timing = true;
  char func_name[100];
  sprintf(func_name, "2d49pt_box_%s_%d_%d_%d_%d_%d_%d_%d_%dschedule", target.c_str(), num_threads, D1,D2, M,N, tile_size_x, tile_size_y, use_schedule);
  stencil.compile_to_source_code_mpi(func_name, target, use_timing);
}
...
/*yxy add : radius=3 3d19pt*/
void bench_3d19pt_star(std::string target, int num_threads, int D1, int D2, int D3, 
            int M, int N, int P, int tile_size_x,  int tile_size_y, int tile_size_z, int use_schedule=1, int is_assigned=0){
  init();
  const int halo_size = 3;
  const int time_window_size = 2;
  ...
  Kernel A( (k,j,i), c0 * B[k-3, j, i] + c1 * B[k+3, j, i] + c2 * B[k, j-3, i] + c3 * B[k, j+3, i] + c4 * B[k, j, i-3] + c5 * B[k, j, i+3] +
  c0 * B[k-2, j, i] + c1 * B[k+2, j, i] + c2 * B[k, j-2, i] + c3 * B[k, j+2, i] + c4 * B[k, j, i-2] + c5 * B[k, j, i+2] + 
  c0 * B[k-1, j, i] + c1 * B[k+1, j, i] + c2 * B[k, j-1, i] + c3 * B[k, j+1, i] + c4 * B[k, j, i-1] + c5 * B[k, j, i+1] + c6 * B[k, j, i], schedule);
  //
  if (use_schedule){
    Axis xo, yo, xi, yi, zo, zi;
    A.tile(tile_size_x, tile_size_y, tile_size_z, xo, xi, yo, yi, zo, zi);
  ...
  const bool use_timing = true;
  char func_name[100];
  sprintf(func_name, "3d19pt_star_%s_%d_%d_%d_%d_%d_%d_%d_%d_%d_%d_%dschedule", target.c_str(), num_threads, D1,D2,D3, M,N,P, tile_size_x, tile_size_y, tile_size_z, use_schedule);
  stencil.compile_to_source_code_mpi(func_name, target, use_timing);
}
...
/*
  yxy add
*/
void bench_3d27pt_box(std::string target, int num_threads, int D1, int D2, int D3, 
          int M, int N, int P, int tile_size_x,  int tile_size_y, int tile_size_z, int use_schedule=1, int is_assigned=0)
{
    init();
    ...
    Kernel A((x,y,z), c0_0 * B[x-1, y-1, z-1] + c0_1 * B[x-1, y-1, z] + c0_2 * B[x-1, y-1, z+1] + 
                      c0_0 * B[x-1, y, z-1] + c0_1 * B[x-1, y, z] + c0_2 * B[x-1, y, z+1] + 
                      c0_0 * B[x-1, y+1, z-1] + c0_1 * B[x-1, y+1, z] + c0_2 * B[x-1, y+1, z+1] + 
                      c0_0 * B[x, y-1, z-1] + c0_1 * B[x, y-1, z] + c0_2 * B[x, y-1, z+1] + 
                      c0_0 * B[x, y, z-1] + c0_1 * B[x, y, z] + c0_2 * B[x, y, z+1] + 
                      c0_0 * B[x, y+1, z-1] + c0_1 * B[x, y+1, z] + c0_2 * B[x, y+1, z+1] + 
                      c0_0 * B[x+1, y-1, z-1] + c0_1 * B[x+1, y-1, z] + c0_2 * B[x+1, y-1, z+1] + 
                      c0_0 * B[x+1, y, z-1] + c0_1 * B[x+1, y, z] + c0_2 * B[x+1, y, z+1] + 
                      c0_0 * B[x+1, y+1, z-1] + c0_1 * B[x+1, y+1, z] + c0_2 * B[x+1, y+1, z+1], schedule);
    if (use_schedule) {
      Axis xo, xi, yo, yi, zo, zi;
      A.tile(tile_size_x, tile_size_y, tile_size_z, xo, xi, yo, yi, zo, zi);
      A.reorder(xo, yo, zo, xi, yi, zi);
    ...
    const bool use_timing = true;
    char func_name[100];
    sprintf(func_name, "3d27pt_box_%s_%d_%d_%d_%d_%d_%d_%d_%d_%d_%d_%dschedule", target.c_str(), num_threads, D1,D2,D3, M,N,P, tile_size_x, tile_size_y, tile_size_z, use_schedule);
    stencil.compile_to_source_code_mpi(func_name, target, use_timing);
}
...
/*
  yxy add
*/
void bench_3d125pt_box(std::string target, int num_threads, int D1, int D2, int D3, 
          int M, int N, int P, int tile_size_x,  int tile_size_y, int tile_size_z, int use_schedule=1, int is_assigned=0)
{
    init();
    ...
    DefVar(y, i32);
    DefVar(z, i32);
    DefVar_Value(c0_0, DT, 0.01);
    DefVar_Value(c0_1, DT, 0.02);
    DefVar_Value(c0_2, DT, 0.03);
    DefVar_Value(c0_3, DT, 0.04);
    DefVar_Value(c0_4, DT, 0.05);

    Kernel A((x, y, z), c0_0 * B[x-2, y-2, z-2] + c0_1 * B[x-2, y-2, z-1] + c0_2 * B[x-2, y-2, z] + c0_3 * B[x-2, y-2, z+1] + c0_4 * B[x-2, y-2, z+2] +
                      c0_0 * B[x-2, y-1, z-2] + c0_1 * B[x-2, y-1, z-1] + c0_2 * B[x-2, y-1, z] + c0_3 * B[x-2, y-1, z+1] + c0_4 * B[x-2, y-1, z+2] +
                      c0_0 * B[x-2, y, z-2] + c0_1 * B[x-2, y, z-1] + c0_2 * B[x-2, y, z] + c0_3 * B[x-2, y, z+1] + c0_4 * B[x-2, y, z+2] +
                      c0_0 * B[x-2, y+1, z-2] + c0_1 * B[x-2, y+1, z-1] + c0_2 * B[x-2, y+1, z] + c0_3 * B[x-2, y+1, z+1] + c0_4 * B[x-2, y+1, z+2] +
                      c0_0 * B[x-2, y+2, z-2] + c0_1 * B[x-2, y+2, z-1] + c0_2 * B[x-2, y+2, z] + c0_3 * B[x-2, y+2, z+1] + c0_4 * B[x-2, y+2, z+2] +
                      c0_0 * B[x-1, y-2, z-2] + c0_1 * B[x-1, y-2, z-1] + c0_2 * B[x-1, y-2, z] + c0_3 * B[x-1, y-2, z+1] + c0_4 * B[x-1, y-2, z+2] +
                      c0_0 * B[x-1, y-1, z-2] + c0_1 * B[x-1, y-1, z-1] + c0_2 * B[x-1, y-1, z] + c0_3 * B[x-1, y-1, z+1] + c0_4 * B[x-1, y-1, z+2] +
                      c0_0 * B[x-1, y, z-2] + c0_1 * B[x-1, y, z-1] + c0_2 * B[x-1, y, z] + c0_3 * B[x-1, y, z+1] + c0_4 * B[x-1, y, z+2] +
                      c0_0 * B[x-1, y+1, z-2] + c0_1 * B[x-1, y+1, z-1] + c0_2 * B[x-1, y+1, z] + c0_3 * B[x-1, y+1, z+1] + c0_4 * B[x-1, y+1, z+2] +
                      c0_0 * B[x-1, y+2, z-2] + c0_1 * B[x-1, y+2, z-1] + c0_2 * B[x-1, y+2, z] + c0_3 * B[x-1, y+2, z+1] + c0_4 * B[x-1, y+2, z+2] +
                      c0_0 * B[x, y-2, z-2] + c0_1 * B[x, y-2, z-1] + c0_2 * B[x, y-2, z] + c0_3 * B[x, y-2, z+1] + c0_4 * B[x, y-2, z+2] +
                      c0_0 * B[x, y-1, z-2] + c0_1 * B[x, y-1, z-1] + c0_2 * B[x, y-1, z] + c0_3 * B[x, y-1, z+1] + c0_4 * B[x, y-1, z+2] +
                      c0_0 * B[x, y, z-2] + c0_1 * B[x, y, z-1] + c0_2 * B[x, y, z] + c0_3 * B[x, y, z+1] + c0_4 * B[x, y, z+2] +
                      c0_0 * B[x, y+1, z-2] + c0_1 * B[x, y+1, z-1] + c0_2 * B[x, y+1, z] + c0_3 * B[x, y+1, z+1] + c0_4 * B[x, y+1, z+2] +
                      c0_0 * B[x, y+2, z-2] + c0_1 * B[x, y+2, z-1] + c0_2 * B[x, y+2, z] + c0_3 * B[x, y+2, z+1] + c0_4 * B[x, y+2, z+2] +
                      c0_0 * B[x+1, y-2, z-2] + c0_1 * B[x+1, y-2, z-1] + c0_2 * B[x+1, y-2, z] + c0_3 * B[x+1, y-2, z+1] + c0_4 * B[x+1, y-2, z+2] +
                      c0_0 * B[x+1, y-1, z-2] + c0_1 * B[x+1, y-1, z-1] + c0_2 * B[x+1, y-1, z] + c0_3 * B[x+1, y-1, z+1] + c0_4 * B[x+1, y-1, z+2] +
                      ...
                      c0_0 * B[x+2, y, z-2] + c0_1 * B[x+2, y, z-1] + c0_2 * B[x+2, y, z] + c0_3 * B[x+2, y, z+1] + c0_4 * B[x+2, y, z+2] +
                      c0_0 * B[x+2, y+1, z-2] + c0_1 * B[x+2, y+1, z-1] + c0_2 * B[x+2, y+1, z] + c0_3 * B[x+2, y+1, z+1] + c0_4 * B[x+2, y+1, z+2] +
                      c0_0 * B[x+2, y+2, z-2] + c0_1 * B[x+2, y+2, z-1] + c0_2 * B[x+2, y+2, z] + c0_3 * B[x+2, y+2, z+1] + c0_4 * B[x+2, y+2, z+2], schedule);
    ...
    const bool use_timing = true;
    char func_name[100];
    sprintf(func_name, "3d125pt_box_%s_%d_%d_%d_%d_%d_%d_%d_%d_%d_%d_%dschedule", target.c_str(), num_threads, D1,D2,D3, M,N,P, tile_size_x, tile_size_y, tile_size_z, use_schedule);
    stencil.compile_to_source_code_mpi(func_name, target, use_timing); 
}
...
/*
  yxy add
*/
void bench_3d343pt_box(std::string target, int num_threads, int D1, int D2, int D3, 
          int M, int N, int P, int tile_size_x,  int tile_size_y, int tile_size_z, int use_schedule=1, int is_assigned=0)
{
    init();
    ...
    DefVar_Value(c0_3, DT, 0.04);
    DefVar_Value(c0_4, DT, 0.05);
    DefVar_Value(c0_5, DT, 0.06);
    DefVar_Value(c0_6, DT, 0.07);

    Kernel A ( (x, y, z), c0_0 * B[x-3,y-3,z-3] + c0_1 * B[x-3,y-3,z-2] + c0_2 * B[x-3,y-3,z-1] + c0_3 * B[x-3,y-3,z] + c0_4 * B[x-3,y-3,z+1] + c0_5 * B[x-3,y-3,z+2] + c0_6 * B[x-3,y-3,z+3] +
                      c0_0 * B[x-3,y-2,z-3] + c0_1 * B[x-3,y-2,z-2] + c0_2 * B[x-3,y-2,z-1] + c0_3 * B[x-3,y-2,z] + c0_4 * B[x-3,y-2,z+1] + c0_5 * B[x-3,y-2,z+2] + c0_6 * B[x-3,y-2,z+3] +
                      c0_0 * B[x-3,y-1,z-3] + c0_1 * B[x-3,y-1,z-2] + c0_2 * B[x-3,y-1,z-1] + c0_3 * B[x-3,y-1,z] + c0_4 * B[x-3,y-1,z+1] + c0_5 * B[x-3,y-1,z+2] + c0_6 * B[x-3,y-1,z+3] +
                      c0_0 * B[x-3,y,z-3] + c0_1 * B[x-3,y,z-2] + c0_2 * B[x-3,y,z-1] + c0_3 * B[x-3,y,z] + c0_4 * B[x-3,y,z+1] + c0_5 * B[x-3,y,z+2] + c0_6 * B[x-3,y,z+3] +
                      c0_0 * B[x-3,y+1,z-3] + c0_1 * B[x-3,y+1,z-2] + c0_2 * B[x-3,y+1,z-1] + c0_3 * B[x-3,y+1,z] + c0_4 * B[x-3,y+1,z+1] + c0_5 * B[x-3,y+1,z+2] + c0_6 * B[x-3,y+1,z+3] +
                      c0_0 * B[x-3,y+2,z-3] + c0_1 * B[x-3,y+2,z-2] + c0_2 * B[x-3,y+2,z-1] + c0_3 * B[x-3,y+2,z] + c0_4 * B[x-3,y+2,z+1] + c0_5 * B[x-3,y+2,z+2] + c0_6 * B[x-3,y+2,z+3] +
                      c0_0 * B[x-3,y+3,z-3] + c0_1 * B[x-3,y+3,z-2] + c0_2 * B[x-3,y+3,z-1] + c0_3 * B[x-3,y+3,z] + c0_4 * B[x-3,y+3,z+1] + c0_5 * B[x-3,y+3,z+2] + c0_6 * B[x-3,y+3,z+3] +
                      c0_0 * B[x-2,y-3,z-3] + c0_1 * B[x-2,y-3,z-2] + c0_2 * B[x-2,y-3,z-1] + c0_3 * B[x-2,y-3,z] + c0_4 * B[x-2,y-3,z+1] + c0_5 * B[x-2,y-3,z+2] + c0_6 * B[x-2,y-3,z+3] +
                      c0_0 * B[x-2,y-2,z-3] + c0_1 * B[x-2,y-2,z-2] + c0_2 * B[x-2,y-2,z-1] + c0_3 * B[x-2,y-2,z] + c0_4 * B[x-2,y-2,z+1] + c0_5 * B[x-2,y-2,z+2] + c0_6 * B[x-2,y-2,z+3] +
                      c0_0 * B[x-2,y-1,z-3] + c0_1 * B[x-2,y-1,z-2] + c0_2 * B[x-2,y-1,z-1] + c0_3 * B[x-2,y-1,z] + c0_4 * B[x-2,y-1,z+1] + c0_5 * B[x-2,y-1,z+2] + c0_6 * B[x-2,y-1,z+3] +
                      c0_0 * B[x-2,y,z-3] + c0_1 * B[x-2,y,z-2] + c0_2 * B[x-2,y,z-1] + c0_3 * B[x-2,y,z] + c0_4 * B[x-2,y,z+1] + c0_5 * B[x-2,y,z+2] + c0_6 * B[x-2,y,z+3] +
                      c0_0 * B[x-2,y+1,z-3] + c0_1 * B[x-2,y+1,z-2] + c0_2 * B[x-2,y+1,z-1] + c0_3 * B[x-2,y+1,z] + c0_4 * B[x-2,y+1,z+1] + c0_5 * B[x-2,y+1,z+2] + c0_6 * B[x-2,y+1,z+3] +
                      c0_0 * B[x-2,y+2,z-3] + c0_1 * B[x-2,y+2,z-2] + c0_2 * B[x-2,y+2,z-1] + c0_3 * B[x-2,y+2,z] + c0_4 * B[x-2,y+2,z+1] + c0_5 * B[x-2,y+2,z+2] + c0_6 * B[x-2,y+2,z+3] +
                      c0_0 * B[x-2,y+3,z-3] + c0_1 * B[x-2,y+3,z-2] + c0_2 * B[x-2,y+3,z-1] + c0_3 * B[x-2,y+3,z] + c0_4 * B[x-2,y+3,z+1] + c0_5 * B[x-2,y+3,z+2] + c0_6 * B[x-2,y+3,z+3] +
                      c0_0 * B[x-1,y-3,z-3] + c0_1 * B[x-1,y-3,z-2] + c0_2 * B[x-1,y-3,z-1] + c0_3 * B[x-1,y-3,z] + c0_4 * B[x-1,y-3,z+1] + c0_5 * B[x-1,y-3,z+2] + c0_6 * B[x-1,y-3,z+3] +
                      c0_0 * B[x-1,y-2,z-3] + c0_1 * B[x-1,y-2,z-2] + c0_2 * B[x-1,y-2,z-1] + c0_3 * B[x-1,y-2,z] + c0_4 * B[x-1,y-2,z+1] + c0_5 * B[x-1,y-2,z+2] + c0_6 * B[x-1,y-2,z+3] +
                      c0_0 * B[x-1,y-1,z-3] + c0_1 * B[x-1,y-1,z-2] + c0_2 * B[x-1,y-1,z-1] + c0_3 * B[x-1,y-1,z] + c0_4 * B[x-1,y-1,z+1] + c0_5 * B[x-1,y-1,z+2] + c0_6 * B[x-1,y-1,z+3] +
                      c0_0 * B[x-1,y,z-3] + c0_1 * B[x-1,y,z-2] + c0_2 * B[x-1,y,z-1] + c0_3 * B[x-1,y,z] + c0_4 * B[x-1,y,z+1] + c0_5 * B[x-1,y,z+2] + c0_6 * B[x-1,y,z+3] +
                      c0_0 * B[x-1,y+1,z-3] + c0_1 * B[x-1,y+1,z-2] + c0_2 * B[x-1,y+1,z-1] + c0_3 * B[x-1,y+1,z] + c0_4 * B[x-1,y+1,z+1] + c0_5 * B[x-1,y+1,z+2] + c0_6 * B[x-1,y+1,z+3] +
                      c0_0 * B[x-1,y+2,z-3] + c0_1 * B[x-1,y+2,z-2] + c0_2 * B[x-1,y+2,z-1] + c0_3 * B[x-1,y+2,z] + c0_4 * B[x-1,y+2,z+1] + c0_5 * B[x-1,y+2,z+2] + c0_6 * B[x-1,y+2,z+3] +
                      c0_0 * B[x-1,y+3,z-3] + c0_1 * B[x-1,y+3,z-2] + c0_2 * B[x-1,y+3,z-1] + c0_3 * B[x-1,y+3,z] + c0_4 * B[x-1,y+3,z+1] + c0_5 * B[x-1,y+3,z+2] + c0_6 * B[x-1,y+3,z+3] +
                      c0_0 * B[x,y-3,z-3] + c0_1 * B[x,y-3,z-2] + c0_2 * B[x,y-3,z-1] + c0_3 * B[x,y-3,z] + c0_4 * B[x,y-3,z+1] + c0_5 * B[x,y-3,z+2] + c0_6 * B[x,y-3,z+3] +
                      c0_0 * B[x,y-2,z-3] + c0_1 * B[x,y-2,z-2] + c0_2 * B[x,y-2,z-1] + c0_3 * B[x,y-2,z] + c0_4 * B[x,y-2,z+1] + c0_5 * B[x,y-2,z+2] + c0_6 * B[x,y-2,z+3] +
                      c0_0 * B[x,y-1,z-3] + c0_1 * B[x,y-1,z-2] + c0_2 * B[x,y-1,z-1] + c0_3 * B[x,y-1,z] + c0_4 * B[x,y-1,z+1] + c0_5 * B[x,y-1,z+2] + c0_6 * B[x,y-1,z+3] +
                      c0_0 * B[x,y,z-3] + c0_1 * B[x,y,z-2] + c0_2 * B[x,y,z-1] + c0_3 * B[x,y,z] + c0_4 * B[x,y,z+1] + c0_5 * B[x,y,z+2] + c0_6 * B[x,y,z+3] +
                      c0_0 * B[x,y+1,z-3] + c0_1 * B[x,y+1,z-2] + c0_2 * B[x,y+1,z-1] + c0_3 * B[x,y+1,z] + c0_4 * B[x,y+1,z+1] + c0_5 * B[x,y+1,z+2] + c0_6 * B[x,y+1,z+3] +
                      c0_0 * B[x,y+2,z-3] + c0_1 * B[x,y+2,z-2] + c0_2 * B[x,y+2,z-1] + c0_3 * B[x,y+2,z] + c0_4 * B[x,y+2,z+1] + c0_5 * B[x,y+2,z+2] + c0_6 * B[x,y+2,z+3] +
                      c0_0 * B[x,y+3,z-3] + c0_1 * B[x,y+3,z-2] + c0_2 * B[x,y+3,z-1] + c0_3 * B[x,y+3,z] + c0_4 * B[x,y+3,z+1] + c0_5 * B[x,y+3,z+2] + c0_6 * B[x,y+3,z+3] +
                      c0_0 * B[x+1,y-3,z-3] + c0_1 * B[x+1,y-3,z-2] + c0_2 * B[x+1,y-3,z-1] + c0_3 * B[x+1,y-3,z] + c0_4 * B[x+1,y-3,z+1] + c0_5 * B[x+1,y-3,z+2] + c0_6 * B[x+1,y-3,z+3] +
                      ...
                      c0_0 * B[x+3,y+1,z-3] + c0_1 * B[x+3,y+1,z-2] + c0_2 * B[x+3,y+1,z-1] + c0_3 * B[x+3,y+1,z] + c0_4 * B[x+3,y+1,z+1] + c0_5 * B[x+3,y+1,z+2] + c0_6 * B[x+3,y+1,z+3] +
                      c0_0 * B[x+3,y+2,z-3] + c0_1 * B[x+3,y+2,z-2] + c0_2 * B[x+3,y+2,z-1] + c0_3 * B[x+3,y+2,z] + c0_4 * B[x+3,y+2,z+1] + c0_5 * B[x+3,y+2,z+2] + c0_6 * B[x+3,y+2,z+3] +
                      c0_0 * B[x+3,y+3,z-3] + c0_1 * B[x+3,y+3,z-2] + c0_2 * B[x+3,y+3,z-1] + c0_3 * B[x+3,y+3,z] + c0_4 * B[x+3,y+3,z+1] + c0_5 * B[x+3,y+3,z+2] + c0_6 * B[x+3,y+3,z+3], schedule);
    ...
    const bool use_timing = true;
    char func_name[100];
    sprintf(func_name, "3d343pt_box_%s_%d_%d_%d_%d_%d_%d_%d_%d_%d_%d_%dschedule", target.c_str(), num_threads, D1,D2,D3, M,N,P, tile_size_x, tile_size_y, tile_size_z, use_schedule);
    stencil.compile_to_source_code_mpi(func_name, target, use_timing); 
}
...
int main(int argc, char * argv[]){
  ...
  if (argc == 2+8+2){
    ...
    /*
      add uh-test-case here
    */

    else if (strcmp(argv[1], "2d5pt_star") == 0) func = bench_2d5pt_star;
    else if (strcmp(argv[1], "2d25pt_box") == 0) func = bench_2d25pt_box;
    else if (strcmp(argv[1], "2d13pt_star") == 0) func = bench_2d13pt_star;
    else if (strcmp(argv[1], "2d49pt_box") == 0) func = bench_2d49pt_box;
   ...
   else if (strcmp(argv[1], "3d27pt_box") == 0)        func = bench_3d27pt_box;
   else if (strcmp(argv[1], "3d125pt_box") == 0)       func = bench_3d125pt_box;

   else if (strcmp(argv[1], "3d19pt_star") == 0)       func = bench_3d19pt_star;
   else if (strcmp(argv[1], "3d343pt_box") == 0)       func = bench_3d343pt_box;
